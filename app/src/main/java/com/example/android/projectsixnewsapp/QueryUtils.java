...
public final class QueryUtils {
    ...
    private static final int MIN_MILLISECONDS = 10000;
    private static final int MAX_MILLISECONDS = 15000;

    private static final String RESPONSE = "response";
    private static final String RESULTS = "results";

    // QueryUtils class referenced from course app - QuakeReport.
    ...
    /**
     * Create a private constructor, QueryUtils object should not be created. This class has static
     * variables and methods which can be accessed directly from here.
     */
    private QueryUtils() {
    }
    ...
    private static List<NewsItem> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                try {
                    publicationDate = (new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss"))
                            .parse(originalPublicationDate);
                } catch (Exception e) {
                ...
                // Create a new {@link NewsItem} object with the title, section name,
                // publication date and url from the JSON response.
                NewsItem news = new NewsItem(title, sectionName, authorFullName, publicationDate,
                        url);

                // Add the new {@link NewsItem} to the list of articles.
                newsList.add(news);
            }
